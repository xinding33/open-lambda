...
func kill() {
	...
	for _, container := range containers {
		if container.Labels[sbmanager.DOCKER_LABEL_CLUSTER] == server.config.Cluster_name {
			...
			kill_opts := docker.KillContainerOptions{ID: cid}
			if err := docker_client.KillContainer(kill_opts); err != nil {
				fmt.Printf("%s\n", err.Error())
			...
			rmv_opts := docker.RemoveContainerOptions{ID: cid}
			if err := docker_client.RemoveContainer(rmv_opts); err != nil {
				fmt.Printf("%s\n", err.Error())
